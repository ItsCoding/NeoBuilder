...
export default function SeoSettingsPage() {
    return (<div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {tabs.map((tab) => (<button key={tab} className="rounded-md border border-slate-200 bg-white px-3 py-2 font-medium text-slate-700 hover:bg-slate-50">
            {tab}
          </button>))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <section className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <label className="block text-sm text-slate-700">
            Title pattern
            <input className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="%page_title% | Acme Bistro"/>
          </label>
          <label className="block text-sm text-slate-700">
            Default description
            <textarea className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm" rows={3} placeholder="Craft a short summary for search and social previews"/>
          </label>
          <label className="block text-sm text-slate-700">
            OG image URL
        ...
        <section className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
          ...
          <textarea className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 font-mono text-sm" rows={6} defaultValue={`User-agent: *\nAllow: /\nSitemap: https://example.com/sitemap.xml`}/>
        ...
        <section className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className="text-sm text-slate-700">
              Opening hours
              <input className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="Mon-Fri 9:00-18:00"/>
            </label>
          </div>
          <div className="rounded-md bg-slate-50 p-3 text-xs text-slate-600">
