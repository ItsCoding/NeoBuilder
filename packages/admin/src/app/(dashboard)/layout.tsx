...
function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      ...
      <aside
        className={`${open ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-40 w-72 shrink-0 border-r border-slate-200 bg-white transition-transform lg:static lg:translate-x-0`}
      >
        <div className="flex items-center justify-between px-5 py-4">
          ...
          <button
            className="rounded-md border border-slate-200 px-2 py-1 text-xs text-slate-600 lg:hidden"
            onClick={onClose}
          >
            Close
          </button>
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-slate-200 bg-white px-6">
      <div className="flex items-center gap-3 text-sm text-slate-600">
        <button
          className="rounded-md border border-slate-200 px-3 py-1 text-sm font-medium text-slate-700 hover:bg-slate-100 lg:hidden"
