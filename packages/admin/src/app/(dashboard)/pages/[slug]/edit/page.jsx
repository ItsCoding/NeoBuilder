...
function InspectorPanel() {
    const { actions, selectedNodeId, selectedDisplayName, selectedProps } = useEditor((state) => {
    ...
    if (!selectedNodeId || !selectedProps) {
        return (<div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
        Select a block to edit its properties.
      </div>);
    }
    return (<div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div>
      ...
      {inspectorSchema
            ...
            .map((field) => (<label key={field.key} className="block text-sm text-slate-700">
            {field.label}
            {field.type === "select" ? (<select className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm" value={String(selectedProps[field.key] ?? "")} onChange={(event) => handleChange(field, event.target.value)}>
                {(field.options ?? []).map((option) => (<option key={option} value={option}>
                    {option}
                  </option>))}
              </select>) : (<input className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2 text-sm" type={field.type === "number" ? "number" : "text"} value={String(selectedProps[field.key] ?? "")} onChange={(event) => handleChange(field, event.target.value)}/>)}
          </label>))}
...
function Palette({ onAdd }) {
    ...
    return (<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-slate-900">Blocks</p>
        <input className="w-40 rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="Search" value={filter} onChange={(event) => setFilter(event.target.value)}/>
      </div>
      <div className="mt-3 space-y-2">
        {filtered.map((block) => (<button key={block.type} className="w-full rounded-lg border border-slate-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" onClick={() => onAdd(block.type)}>
            <div className="flex items-center justify-between">
...
function TemplatePicker({ onApply }) {
    return (<div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-2 space-y-2">
        {templateLibrary.map((template) => (<button key={template.id} className="w-full rounded-md border border-slate-200 px-3 py-2 text-left text-sm hover:bg-slate-50" onClick={() => onApply(template.id)}>
...
function EditorSurface({ slug }) {
    const { actions, query } = useEditor();
    const [device, setDevice] = useState("desktop");
    const [frameContent, setFrameContent] = useState(starterBlocks);
    ...
    const addBlock = (type) => {
        ...
        const Component = definition ? craftResolver[definition.type] : undefined;
    ...
    const applyTemplate = (templateId) => {
        const template = templateLibrary.find((tpl) => tpl.id === templateId);
        if (!template)
            return;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex rounded-full border border-slate-200 bg-white p-1 text-sm shadow-sm">
            {["desktop", "tablet", "mobile"].map((d) => (<button key={d} className={`rounded-full px-3 py-1 ${device === d ? "bg-slate-900 text-white" : "text-slate-700"}`} onClick={() => setDevice(d)}>
                {d}
              </button>))}
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[280px,1fr,320px]">
        <aside className="space-y-3">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm text-sm text-slate-700">
            ...
            <div className="mt-2 space-y-2">
              {["main-header", "promo-bar", "footer"].map((sectionKey) => (<button key={sectionKey} className="w-full rounded-md border border-slate-200 px-3 py-2 text-left text-sm hover:bg-slate-50" onClick={() => addBlock("GlobalSectionBlock")}>
                  {sectionKey}
                </button>))}
            </div>
        ...
        <section className="space-y-3">
          <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-slate-700">
              ...
              <button className="rounded-md border border-slate-200 px-3 py-1 text-xs text-slate-700 hover:bg-slate-100" onClick={() => toast.info("Craft.js serialized JSON is stored in Page.draftContent/Page.publishedContent.")}>
                Persistence details
              </button>
            </div>
          </div>
...
export default function PageEditor({ params }) {
    return (<Editor resolver={craftResolver} enabled>
