    ...
    return (<div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex gap-3">
          ...
          <a className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700" href="/pages/new">
            New page
          </a>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:col-span-3">
          <div className="flex flex-wrap gap-3 text-sm">
            ...
            <input className="min-w-[200px] rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="Search title or slug"/>
          ...
          <div className="overflow-hidden rounded-lg border border-slate-200">
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {data.map((row) => (<tr key={row.slug}>
                    <td className="px-4 py-2 font-medium text-slate-900">{row.title}</td>
                    <td className="px-4 py-2 text-slate-600">{row.slug}</td>
                    <td className="px-4 py-2">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles[row.status]}`}>
                        {row.status}
                      </span>
                    </td>
        ...
        <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <ul className="space-y-2 text-sm text-slate-700">
            {versions.map((v) => (<li key={v.version} className="rounded-md border border-slate-200 p-2">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          {data
            ...
            .map((item) => (<div key={item.slug} className="rounded-md border border-slate-200 p-3 text-sm">
                <p className="font-semibold text-slate-900">{item.title}</p>
                {item.scheduledPublishAt && (<p className="text-slate-700">Publish: {item.scheduledPublishAt}</p>)}
                {item.scheduledUnpublishAt && (<p className="text-slate-700">Unpublish: {item.scheduledUnpublishAt}</p>)}
