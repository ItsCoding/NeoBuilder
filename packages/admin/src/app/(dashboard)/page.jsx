    ...
    return (<div className="space-y-6">
      <header className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex gap-3 text-sm">
          <a className="rounded-md bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-700" href="/pages/new">
            Create page
          </a>
          <a className="rounded-md border border-slate-200 px-4 py-2 font-medium text-slate-700 hover:bg-slate-100" href="/media">
            Upload media
          </a>
        </div>
      </header>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (<div key={metric.label} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            {activity.map((item) => (<li key={item} className="flex items-center gap-2">
